...
import {TokenCountStrategy} from '../../../services/limit-strategies';
...
describe('TokenCountStrategy Unit', function () {
  let strategy: TokenCountStrategy;
  ...
  it('should throw HttpErrors.InternalServerError when token limit strategy is not configured properly', async () => {
    ...
    strategy = new TokenCountStrategy(
      {} as AIIntegrationConfig,
      user,
      chatRepo,
    );
  ...
  it('should not throw error when token usage is within limit', async () => {
    ...
    strategy = new TokenCountStrategy(config, user, chatRepo);
