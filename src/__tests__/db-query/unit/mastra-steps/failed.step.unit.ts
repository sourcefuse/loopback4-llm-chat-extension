...
describe('failedStep (Mastra)', function () {
  ...
  beforeEach(() => {
    writerSpy = sinon.spy();
    context = {emit: writerSpy} as unknown as MastraDbQueryContext;
  });

  it('emits a ToolStatus.Failed event', async () => {
    const state = {} as DbQueryState;
    await runStep(failedStep, {state, context});
  ...
  it('returns the existing replyToUser when already set', async () => {
    ...
    const result = await runStep(failedStep, {state, context});
  ...
  it('works without a writer in context', async () => {
    ...
    const state = {} as DbQueryState;
    const result = await runStep(failedStep, {state, context: ctxNoWriter});
