...
// A `@graphNode(key)` class is stamped `isNODE: true` + `GRAPH_NODE: <key>`, so
// a workflow node is discovered by tag (`context.findByTag({GRAPH_NODE: key})`)
// and resolved from DI — the BaseGraph._getNodeFn seam. This is what makes
// individual nodes overrideable by a host app: rebind the tagged class.
