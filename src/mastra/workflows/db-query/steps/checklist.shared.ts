...
export function collectDomainRules(
  rc: MastraRc | undefined,
  tables: string[],
): DomainRuleContext {
  const globalContext = getGlobalContext(rc);
  let tableContext: string[] = [];
  let schemaText = '';
  const schemaStore = getSchemaStore(rc);
  const schemaHelper = getSchemaHelper(rc);
...
export async function selectDomainRules(args: {
  ...
  if (!llm) return [];
  const {rules, schemaText} = collectDomainRules(rc, tables);
