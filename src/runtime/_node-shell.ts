 ...
 * constraint), and whose `execute` delegates to a `@graphNode(resolverKey)` class
 ...
 */
export function makeNodeShell(opts: {
  id: string;
  ...
  return createStep({
    ...
    execute: async ctx => {
      ...
      return stepImpl.execute(ctx as unknown as GraphNodeCtx);
    },
  });
}
