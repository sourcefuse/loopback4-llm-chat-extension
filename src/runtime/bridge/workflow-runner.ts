...
export class WorkflowRunner {
  ...
  async *run(
    ...
    // Publish the per-request RESOLVED model tiers as container bindings so
    // step classes `@inject` a ready-to-call model (the resolver builds steps
    // from this.lb4Ctx). The configs were resolved once in resolveRequestContextShape.
    this.bindRuntimeModels(rcShape);
